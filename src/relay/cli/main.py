...
def up(
    ...
    """Start the swarm's workers as detached processes (and local Redis if needed)."""
    from relay.cli import procs, redisctl

    project = project.expanduser().resolve()
    if not (project / "relay.toml").exists():
        console.print(f"[red]✗[/red] {project} is not initialized — run: relay init --project {project}")
        raise typer.Exit(1)
...
def init(
    ...
    """Prepare a project: git repo, .gitignore, permission profiles, relay.toml."""
