...
def _pass_behaviour_gates(swarm: MiniSwarm, bid: str) -> None:
    ...
    for g in gates:
        ...
        swarm.publisher.send(role, "coordinator", "gate.verdict",
...
def _accept(swarm: MiniSwarm, bid: str) -> None:
    ...
    swarm.publisher.send("specifier", "coordinator", "work.acceptance_verdict",
...
def test_fully_gated_iteration_with_mutation_security_and_pr(client, publisher) -> None:
    ...
    publisher.send("qa", "coordinator", "gate.verdict",
    ...
    publisher.send("security", "coordinator", "gate.verdict",
    ...
    publisher.send("interpreter", "coordinator", "plan.pr_approved",
                   {"iteration_id": "I1", "gate_id": "gate-01J5AB3CDEF4GH5JK6MN7PQ8RS"})
    swarm.pump()
    (pr,) = swarm.sent("plan.pr_opened")
...
def test_gate_failure_causes_rework_with_findings(client, publisher) -> None:
    ...
    swarm.pump()
    rework = swarm.sent("work.rework_requested")
    assert len(rework) == 1
    assert rework[0].payload["attempt"] == 2
    assert swarm.behaviour("I1.S1.B1").state == BehaviourState.BUILD_DISPATCHED
    assert len(swarm.sent("work.judgement_requested")) == 0  # no judgement past a failed gate
...
def test_mutation_gate_failure_reworks_and_reruns_mutation(client, publisher) -> None:
    ...
    assert len(swarm.sent("plan.story_done")) == 0
...
def test_characterization_red_verification_is_inverted(client, publisher) -> None:
    ...
    publisher.send("specifier", "coordinator", "work.spec_ready",
...
def test_recon_runs_first_and_risk_areas_block_uncharacterized_builds(client, publisher) -> None:
    ...
    mini.pump()
    assert len(mini.sent("work.recon_requested")) == 1  # recon before any roadmap
    publisher.send("analyst", "interpreter", "work.recon_report",
                   {"brief_path": "docs/relay/codebase-brief.md",
                    "risk_areas": ["src/x.py"], "coverage_summary": "untested"})
    publisher.send("interpreter", "coordinator", "plan.roadmap_committed",
                   {"roadmap": ROADMAP, "intake": {"mode": "legacy"}})
    publisher.send("interpreter", "coordinator", "plan.iteration_started",
                   {"iteration_id": "I1"})
    mini.pump()
    ...
    decision = mini.sent("plan.owner_decision_needed")[-1]
    assert "characterization" in decision.payload["reason"]
    assert len(mini.sent("work.build_requested")) == 0  # fail closed
